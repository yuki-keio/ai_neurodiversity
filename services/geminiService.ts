...
export const generateQuestionSuggestions = async (
  ...
  const prompt = `
あなたは、ユーザーがニューロダイバージェント支援チャットボットにどのような質問をすれば良いか、質問のアイデアを提案するAIです。支援チャットボットは、他のニューロダイバージェントの当事者の経験則を体系化した27のパターンに基づいてアドバイスを提供します。

以下の情報を参考にして、ユーザーが支援チャットボットに対して次に尋ねそうな、または尋ねると良さそうな質問の候補を2個ほど提案してください。
質問は具体的で、簡潔なものにしてください。チャットボットが他のニューロダイバージェントの当事者の経験則パターンを引用することで答えられる形が望ましいです。
